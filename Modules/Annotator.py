    ...
    def _upload_results(self):
        ...
        if not args.Practice:
            ...
            if not os.path.exists(projFileManager.localLabeledFramesFile):
                print(
                    projFileManager.localLabeledFramesFile + 'does not exist. Did you annotate any new frames? Quitting...')
            else:
                ...
                old_DT.to_csv(anFileManager.localBoxesAnnotationFile, sep=',',
                              columns=['ProjectID', 'Framefile', 'Nfish', 'Sex', 'Box', 'User', 'DateTime'])
                ...
                for row in newAnn_DT.itertuples():
                    ...
                    if not os.path.exists(anFileManager.localBoxedImagesProjDir + row.Framefile):
                        output = subprocess.run(['cp', projFileManager.localManualLabelFramesDir + row.Framefile,
                                                 anFileManager.localBoxedImagesProjDir], stderr=subprocess.PIPE,
                                                encoding='utf-8')
                        if output.stderr != '':
                            print(output.stderr)
                            raise Exception
                ...
                subprocess.run(
                    ['mv', projFileManager.localLabeledFramesFile, projFileManager.localLabeledFramesFile + '.backup'],
                    stderr=subprocess.PIPE)
                subprocess.run(
                    ['rclone', 'copy', projFileManager.cloudLabeledFramesFile, projFileManager.localAnalysisDir],
                    stderr=subprocess.PIPE)
                ...
                if os.path.exists(projFileManager.localLabeledFramesFile):
                    ...
                    proj_DT = newAnn_DT
                proj_DT.to_csv(projFileManager.localLabeledFramesFile, sep=',',
                               columns=['ProjectID', 'Framefile', 'Nfish', 'Sex', 'Box', 'User', 'DateTime'])
                output = subprocess.run(
                    ['rclone', 'copy', projFileManager.localLabeledFramesFile, projFileManager.cloudAnalysisDir],
...
if not args.Practice:
    ...
    if not os.path.exists(projFileManager.localLabeledFramesFile):
        ...
        subprocess.run(
            ['mv', projFileManager.localLabeledFramesFile, projFileManager.localLabeledFramesFile + '.backup'],
            stderr=subprocess.PIPE)
        subprocess.run(['rclone', 'copy', projFileManager.cloudLabeledFramesFile, projFileManager.localAnalysisDir],
                       stderr=subprocess.PIPE)
